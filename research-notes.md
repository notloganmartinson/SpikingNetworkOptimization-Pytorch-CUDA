...
### 4. Chaotic Dynamics and Simulation Jitter
*   **Original:** To combat the inherent chaotic variance of balanced spiking networks, the original authors ran their simulations for **140 biological seconds** to obtain highly stable, averaged statistics.
*   **FasterSNO:** To keep the simulation fast enough to run on a consumer GPU, this implementation simulates **10 biological seconds**. While `FasterSNO` is now structurally identical to the paper (implementing a true **Spatial Balanced Network (SBN)** with extremely sparse 1-4% connectivity), plugging the exact optimal parameters from the paper will produce statistically identical regimes, but not the exact decimal values. This is due to the reduced simulation length and different random wiring seeds causing statistics to "jitter" around the true mean. A full Bayesian search is recommended to center your unique network's jitter around the target data.
